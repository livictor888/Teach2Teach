...
const pureScriptSelect = (selector) => {
  ...
  let optionValues = {
    [document.querySelector(selector).getAttribute("id")]: eval(
      document.querySelector(selector).getAttribute("data-multiSelect")
    ),
  };

  let isMax = {
    [document.querySelector(selector).getAttribute("id")]: eval(
      document.querySelector(selector).getAttribute("data-max")
    ),
  };
    ...
    function singleSelect() {
      ...
      let html = `
            <div class="directorist-select__label">
                <div class="directorist-select__label--text">${
                  option[0].text
                }</div>
            ...
            <div class="directorist-select__dropdown">
                <input class='directorist-select__search ${
                  isSearch
                    ? "directorist-select__search--show"
                    : "directorist-select__search--hide"
                }' type='text' class='value' placeholder='Filter Options....' />
      ...
      option.forEach((el, index) => {
        ...
        if (el.hasAttribute("selected")) {
          selectTrigger.innerHTML =
            el.innerHTML + '<img src="assets/svg/angle-down-solid.svg" alt="">';
        }
      });
      ...
      document.body.addEventListener("click", (event) => {
        ...
        sibling
          .querySelector(".directorist-select__dropdown")
          .classList.remove("directorist-select__dropdown-open");
        input.value = "";
      });
      ...
      selectTrigger.addEventListener("click", (e) => {
        ...
        sibling
          .querySelector(".directorist-select__dropdown")
          .classList.toggle("directorist-select__dropdown-open");
        ...
        elem.forEach((el, key) => {
          ...
          item2 += `<li><span class="directorist-select-dropdown-text">${
            el.text
          }</span> <span class="directorist-select-dropdown-item-icon"><img src="${attribute}" style="${
            attribute == null && { display: "none" }
          } " /><b class="${attribute2}"></b></b></span></li>`;
        });
        item2 += "</ul>";
        ...
        li.forEach((el, index) => {
          el.addEventListener("click", (event) => {
            ...
            sibling
              .querySelector(".directorist-select__dropdown")
              .classList.remove("directorist-select__dropdown-open");
            item.querySelector(".directorist-select__label").innerHTML =
              el.querySelector(".directorist-select-dropdown-text")
                .textContent +
              '<img src="assets/svg/angle-down-solid.svg" alt="">';
          });
        });
      });
    ...
    function multiSelects() {
      let defaultValues = {
        [document
          ...
          .querySelector(selector)
          .getAttribute("data-default")
          ? eval(document.querySelector(selector).getAttribute("data-default"))
          : [],
      };
      const arraySelector = item.getAttribute("id");
      ...
      let html = `
            <div class="directorist-select__label">
                ...
                <input type="text" class='directorist-select__search ${
                  isSearch
                    ? "directorist-select__search--show"
                    : "directorist-select__search--hide"
                }' type='text' class='value' placeholder='Filter Options....' />
      ...
      function insertSearchItem() {
        item.querySelector(
          ".directorist-select__selected-list"
        ).innerHTML = defaultValues[arraySelector]
          .map(
            (item) =>
              `<span class="directorist-select__selected-list--item">${item}&nbsp;&nbsp;<a href="#" data-key="${item}" class="directorist-item-remove"><i class="fa fa-times"></i></a></span>`
          )
          .join("");
      }
      ...
      document.body.addEventListener("click", (event) => {
        if (
          event.target == button ||
          event.target.closest(".directorist-select__container")
        ) {
          return;
        } else {
          sibling
      ...
      button.addEventListener("click", (e) => {
        ...
        document
          .querySelectorAll(".directorist-select__dropdown")
          .forEach((el) =>
            el.classList.remove("directorist-select__dropdown-open")
          );
        e.target
          .closest(".directorist-select__container")
          .querySelector(".directorist-select__dropdown")
          .classList.add("directorist-select__dropdown-open");
        ...
        li.forEach((element, index) => {
          ...
          if (
            defaultValues[arraySelector].includes(
          ...
          value.addEventListener("keyup", (event) => {
            ...
            if (event.keyCode === 13) {
              if (isMax[arraySelector]) {
                if (
                  defaultValues[arraySelector].length <
                  parseInt(isMax[arraySelector])
                ) {
                  if (
                    !defaultValues[arraySelector].includes(
                      event.target.value
                  ...
                  if (e.target.closest(".directorist-select")) {
                    e.target
                      .closest(".directorist-select")
                      .querySelector(".directorist-select__container")
                      .classList.add("directorist-error");
                    e.target
                      .closest(".directorist-select")
                      .querySelector(
                        ".directorist-error__msg"
                      ).innerHTML = `Max ${isMax[arraySelector]} Items Added `;
                  }
                }
              } else {
                if (
                  !defaultValues[arraySelector].includes(event.target.value) &&
                  event.target.value !== ""
                ) {
                  defaultValues[arraySelector].push(event.target.value);
                  optionValues[arraySelector].push(event.target.value);
            ...
            li.forEach((element, index) => {
              ...
              element.addEventListener("click", (event) => {
                sibling
                  .querySelector(".directorist-select__dropdown--inner")
                  .classList.remove("directorist-select__dropdown.open");
              });
            });
          });
        ...
        eventDelegation("click", "li", function (e) {
          ...
          if (isMax[closestId] === null && defaultValues[closestId]) {
            defaultValues[closestId].filter((item) => item == index).length ===
              0 && defaultValues[closestId].push(index);
            ...
            if (defaultValues[closestId])
              if (
                defaultValues[closestId].length < parseInt(isMax[closestId])
              ) {
                defaultValues[closestId].filter((item) => item == index)
      ...
      eventDelegation("click", ".directorist-item-remove", function (e) {
        ...
        defaultValues[closestId] =
          defaultValues[closestId] &&
          defaultValues[closestId].filter(
            (item) => item != e.target.getAttribute("data-key")
          );
        if (
          (defaultValues[closestId] && defaultValues[closestId].length) <
          (isMax[closestId] && parseInt(isMax[closestId]))
        ) {
          e.target
            .closest(".directorist-select")
            .querySelector(".directorist-select__container")
            .classList.remove("directorist-error");
          e.target
            .closest(".directorist-select")
            .querySelector(".directorist-error__msg").innerHTML = "";
        }
        ...
        li.forEach((element, index) => {
          ...
          element.classList.add("directorist-select-item-hide");
          if (
            defaultValues[closestId].includes(element.getAttribute("data-key"))
          ) {
            element.classList.add("directorist-select-item-show");
            element.classList.remove("directorist-select-item-hide");
          }
